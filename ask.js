...
- Formatting: write in short paragraphs separated by a blank line (a real newline character between them, not just a space). If it helps clarity, use a short bulleted list with lines starting "- ". Wrap any code, variable names, or literal values (e.g. \`x\`, \`3\`, \`nextInt()\`) in single backticks, and use triple backticks for multi-line code. Never write two separate words or tokens with no space between them.
